...
|---|---:|---:|---:|---|
| Productie (standaard) | 512 MiB | 640 MiB | 320 MiB | `deploy/base/backend-memory.yaml` |
| Acceptatie | 320 MiB | 448 MiB | 192 MiB | `deploy/overlays/acceptance/backend-memory.yaml` |

De bestaande garbage collector blijft expliciet behouden: `-XX:+UseSerialGC`. We zetten geen vaste `-Xms`.
