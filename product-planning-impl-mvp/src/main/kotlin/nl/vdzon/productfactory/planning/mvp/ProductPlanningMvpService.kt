...
class ProductPlanningMvpService(
    ...
    override fun recordStoryVerification(command: RecordStoryVerificationCommand) {
        ...
        if (command.passed) maybeQueueEpicVerification(story.epicId)
    ...
    private fun finishSession(id: ProcessSessionId, summary: String, publications: List<SourceReference>) { val now = clock.instant(); jdbc.update("UPDATE pf_planning_process_session SET status='SUCCEEDED',phase='COMPLETED',active_product_id=NULL,publications_json=?,result_summary=?,blocked_reason=NULL,error_code=NULL,call_claimed_until=NULL,finished_at=?,updated_at=? WHERE id=?", mapper.writeValueAsString(publications), summary.take(2000), now, now, id.value) }
