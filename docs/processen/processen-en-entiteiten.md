...
| Product Advisor | versiegebonden commands voor gesprek, bericht, requestbevestiging, retry en gerichte uitwerking | gesprekken, requests, persoonlijke acties en notificaties per bevoegde gebruiker |
...
| antwoord op een agentvraag | `StakeholderQuestion` gekoppeld aan `Meeting` of een gericht `DesignWorkItem` | maakt vraag, antwoord, vragende rol, geadresseerde gebruiker en bronproces controleerbaar |
...
|---|---|---|---|---|
| `Product` | productmodule | globale Stakeholder of productbediening | alle processen en frontend | productidentiteit, status `ACTIVE` of `INACTIVE`, expliciete dispatchinginstelling en epicgoedkeuring `AUTOMATIC` of `MANUAL` |
...
| `ProductConversation` en bericht | Product Advisor | bevoegde product owner of factory owner | bevoegde productgebruikers en frontend | duurzaam geversioneerd gesprek; berichten zijn append-only met afzender `USER`, `PRODUCT_ADVISOR` of `SYSTEM` |
| `ProductRequest` en versie | Product Advisor | advisor stelt voor; product owner bevestigt exact één versie | ontwerp, dispatcher en frontend | onveranderlijke voorstelversies met route `HOTFIX`, `BUGFIX` of `EPIC_CANDIDATE`, bewijs, exacte Git-SHA, scope en grenzen |
| `DesignWorkItem` | Product Advisor | bevestigde epickandidaat | Productontwerp en frontend | gerichte queue die exact hetzelfde item hervat na antwoord op een gekoppelde gebruikersvraag |
| `ProductMembership` | identiteits- en autorisatielaag | factory owner | centrale autorisatie en beheer-UI | geversioneerd actief of ingetrokken `PRODUCT_OWNER`-lidmaatschap met historie |
| `ProductNotification` | Product Advisor | deterministische domeineffecten | uitsluitend geadresseerde gebruiker | persoonlijke, versiegebonden in-appmelding zonder geheime payload |
| `Epic` | Productontwerp | Productplanning vraagt planning/statusovergangen of verdere uitwerking; Kwaliteitsbewaking registreert uitkomst; Stakeholder kan goedkeuren, terugsturen, intrekken of annuleren | ontwerp, planning, kwaliteit en frontend | metadata `id`, `productId`, `version` en status, opgeslagen `title` en `summary`, probleem, oplossing, concrete bron-/ingestieroute, richtinggevende UX-artifacts, acceptatiecriteria, behapbaarheid en eventuele vrije verfijningsreden; status omvat `NEEDS_RESEARCH`, `NEEDS_REFINEMENT`, `AWAITING_APPROVAL`, `AVAILABLE`, `IN_PLANNING`, `ACTIVE`, `VERIFYING`, `COMPLETED`, `NOT_SUCCESSFUL`, `CANCELLED`, `SUPERSEDED` of `WITHDRAWN` |
